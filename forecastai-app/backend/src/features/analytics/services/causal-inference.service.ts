...
export class CausalInferenceEngine {
  analyzeRevenueDrivers(data: any[], channelForecasts: any, budgets: Record<string, number>): CausalAnalysis {
    const drivers: CausalFactor[] = [];
    const totalRevenue = Object.values(channelForecasts || {}).reduce((a: number, fc: any) => a + (fc.p50_revenue || 0), 0);
  ...
  private generateRecommendations(drivers: CausalFactor[]): string[] {
    const recs: string[] = [];
    ...
    if (neg.length) recs.push(`Review ${neg.map(d => d.name.split(' ')[0]).join(', ')} - underperforming ROAS targets`);
    return recs;
  }

  private identifyRisks(drivers: CausalFactor[], channelForecasts: any): string[] {
    const risks: string[] = [];
    if (drivers[0]?.impact > 50) risks.push(`High concentration (${drivers[0].impact.toFixed(0)}%) - diversify to reduce risk`);
    ...
    for (const [channel, forecast] of Object.entries(channelForecasts || {})) {
      ...
      if ((p90 - p10) / p50 > 0.5) risks.push(`High uncertainty in ${channel} forecasts`);
    }
    return risks;
  }

  private buildCausalGraph(drivers: CausalFactor[]): Record<string, string[]> {
    const graph: Record<string, string[]> = { Revenue: drivers.map(d => d.name) };
    for (const d of drivers) graph[d.name] = ['Revenue'];
    return graph;
  }
}
