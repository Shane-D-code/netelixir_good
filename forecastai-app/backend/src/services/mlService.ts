...
function prophetForecast(values: number[], h: number): number[] {
  ...
  const dowEffects: number[] = new Array(7).fill(0);
...
function randomForestPredict(values: number[], features: Record<string, number[]>, h: number): number[] {
  const n = values.length;
  if (n < 14) return simpleForecast(values, h);
  ...
  for (let i = 0; i < h; i++) {
    let sum = 0;
    for (let t = 0; t < nEstimators; t++) sum += forecasts[t][i];
...
function computePercentiles(simulations: number[][]): {
  ...
  const p10: number[] = []; const p25: number[] = []; const p50: number[] = [];
  const p75: number[] = []; const p90: number[] = [];
...
export function runForecastPipeline(
  ...
  channel_forecasts: Record<string, {
    channel: string; median: number[]; p10: number[]; p90: number[];
    p25: number[]; p75: number[]; dates: string[];
    historical: number[]; historical_dates: string[]; total_revenue: number;
  }>;
  total_forecast: {
    median: number[]; p10: number[]; p90: number[]; p25: number[]; p75: number[];
