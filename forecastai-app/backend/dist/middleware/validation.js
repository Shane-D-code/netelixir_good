...
exports.validateForecastRequest = validateForecastRequest;
exports.validateBudgetSimulation = validateBudgetSimulation;
exports.validateCSVUpload = validateCSVUpload;
...
function parseBodyField(req, field) {
    ...
    if (typeof val === 'string') {
        try {
            ...
            throw new errorHandler_1.ValidationError(`${field} must be a valid JSON object`);
        }
    }
}
function validateForecastRequest(req, _res, next) {
    if (!req.body || Object.keys(req.body).length === 0) {
        throw new errorHandler_1.ValidationError('Request body is missing. Ensure the request is sent as multipart/form-data with a file and parameters');
    }
    ...
    if (!channel_budgets || typeof channel_budgets !== 'object') {
        throw new errorHandler_1.ValidationError('channel_budgets is required and must be an object');
    }
    const normalized = {};
    for (const [channel, budget] of Object.entries(channel_budgets)) {
        ...
        if (!VALID_CHANNELS.includes(normalizedName)) {
            throw new errorHandler_1.ValidationError(`Invalid channel: "${channel}". Valid channels: ${VALID_CHANNELS.join(', ')}`);
        }
        if (typeof budget !== 'number' || budget < 0) {
            throw new errorHandler_1.ValidationError(`Budget for "${channel}" must be a non-negative number, got ${typeof budget} ${budget}`);
        }
        normalized[normalizedName] = budget;
    }
    req.body.channel_budgets = normalized;
    if (forecast_days && (forecast_days < 7 || forecast_days > 365)) {
        throw new errorHandler_1.ValidationError(`forecast_days must be between 7 and 365, got ${forecast_days}`);
    }
    if (confidence_level && (confidence_level < 0.5 || confidence_level > 0.99)) {
        throw new errorHandler_1.ValidationError(`confidence_level must be between 0.5 and 0.99, got ${confidence_level}`);
    }
    if (n_simulations && (n_simulations < 100 || n_simulations > 10000)) {
        throw new errorHandler_1.ValidationError(`n_simulations must be between 100 and 10000, got ${n_simulations}`);
    }
    next();
}
function validateBudgetSimulation(req, _res, next) {
    ...
    if (percentage_change === undefined || percentage_change < -100 || percentage_change > 200) {
        throw new errorHandler_1.ValidationError(`percentage_change must be between -100 and 200, got ${percentage_change}`);
    }
    if (!current_budgets || typeof current_budgets !== 'object') {
        throw new errorHandler_1.ValidationError('current_budgets is required and must be an object');
    }
    if (base_revenue === undefined || base_revenue === null || typeof base_revenue !== 'number') {
        throw new errorHandler_1.ValidationError(`base_revenue is required and must be a number, got ${typeof base_revenue}`);
    }
    next();
}
function validateCSVUpload(req, _res, next) {
    if (!req.file) {
        throw new errorHandler_1.ValidationError('CSV file is required');
    }
    if (!req.file.originalname.endsWith('.csv')) {
        throw new errorHandler_1.ValidationError(`Only CSV files are accepted, got "${req.file.originalname}"`);
    }
    if (req.file.size > 10 * 1024 * 1024) {
        const sizeMB = (req.file.size / (1024 * 1024)).toFixed(2);
        throw new errorHandler_1.ValidationError(`File size must be less than 10MB, got ${sizeMB}MB`);
    }
    next();
}
