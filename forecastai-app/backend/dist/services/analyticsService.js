...
class AnalyticsService {
    ...
    computePerformanceMetrics(data, channelBudgets) {
        const byChannel = {};
        for (const row of data) {
            if (!byChannel[row.channel])
                byChannel[row.channel] = [];
        ...
        for (const channel of channels) {
            ...
            if (values.length < forecastDays + 14)
                continue;
            const result = (0, mlService_1.runForecastPipeline)(data.filter(r => r.channel === channel), { [channel]: channelBudgets[channel] || 1000 }, forecastDays, 0.8, 500, {
    ...
    generateAccuracyReport(metrics) {
        const lines = [
            '─'.repeat(50),
