...
exports.roasOptimizer = exports.ROASOptimizer = void 0;
class ROASOptimizer {
    constructor() {
        this.defaultConstraint = { minROAS: 2.0, targetROAS: 3.0, maxBudget: 200000 };
    }
    optimizeForROAS(channelForecasts, currentBudgets, constraints = {}) {
        const optimized = [];
    ...
    generateRecommendations(optimized) {
        return optimized.map(opt => {
            if (opt.adjustment > 0)
                return `Increase ${opt.channel} by ${opt.adjustment}% - strong ROAS (${opt.expectedROAS.toFixed(2)}x)`;
            if (opt.adjustment < 0)
                return `Reduce ${opt.channel} by ${Math.abs(opt.adjustment)}% - below target ROAS (${opt.expectedROAS.toFixed(2)}x)`;
            return `${opt.channel} allocation is optimal`;
        });
    }
    calculateROASConfidenceInterval(revenue, budget, confidenceLevel = 0.8) {
...
exports.ROASOptimizer = ROASOptimizer;
exports.roasOptimizer = new ROASOptimizer();
//# sourceMappingURL=roasOptimizer.js.map