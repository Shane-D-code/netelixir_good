...
exports.seasonalityDetector = exports.SeasonalityDetector = void 0;
class SeasonalityDetector {
    ...
    aggregateDailyRevenue(data) {
        const map = new Map();
        for (const row of data) {
            const date = (row.date || '').split('T')[0];
            if (date)
                map.set(date, (map.get(date) || 0) + (row.revenue || 0));
        }
        return map;
    }
    calculateWeeklyPattern(dailyRevenue) {
    ...
    calculateMonthlyPattern(dailyRevenue) {
    ...
    detectHolidayEffects(_dailyRevenue) {
    ...
    generateRecommendations(weeklyPattern, monthlyPattern) {
...
exports.SeasonalityDetector = SeasonalityDetector;
exports.seasonalityDetector = new SeasonalityDetector();
//# sourceMappingURL=seasonalityDetector.js.map