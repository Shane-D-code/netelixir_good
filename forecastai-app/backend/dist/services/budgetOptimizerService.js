...
exports.budgetOptimizerService = exports.BudgetOptimizerService = void 0;
...
class BudgetOptimizerService {
    simulateBudgetChange(currentBudgets, targetChannel, percentageChange, baseForecastRevenue) {
        ...
        let impact = 'neutral';
        if (newRoas > currentRoas * 1.1)
            impact = 'positive';
        else if (newRoas < currentRoas * 0.9)
            impact = 'negative';
        return {
    ...
    simulateMultipleScenarios(currentBudgets, baseRevenue, scenarios) {
        return scenarios.map(s => this.simulateBudgetChange(currentBudgets, s.channel, s.pctChange, baseRevenue));
    }
    optimizeAllocation(currentBudgets, totalBudget, historicalRoas) {
        ...
        for (const channel of Object.keys(currentBudgets)) {
            ...
            if (changePercent > 10)
                expectedImpact = 'Increase budget for higher returns';
            else if (changePercent < -10)
                expectedImpact = 'Reduce budget to improve efficiency';
            else
                expectedImpact = 'Maintain current allocation';
    ...
    getRevenueCurve(currentBudgets, baseRevenue, channel, pctRange = Array.from({ length: 16 }, (_, i) => -50 + i * 10), _steps = 16) {
        const elasticity = CHANNEL_ELASTICITIES[channel] || 0.5;
        const budgets = [];
        const revenues = [];
        const elasticities = [];
...
exports.budgetOptimizerService = new BudgetOptimizerService();
//# sourceMappingURL=budgetOptimizerService.js.map