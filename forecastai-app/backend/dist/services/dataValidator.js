...
class DataValidator {
    validateCampaignConsistency(data) {
        const errors = [];
        const warnings = [];
        const fixedIssues = [];
        if (data.length === 0) {
        ...
        for (const row of data) {
            ...
            if (row.revenue < 0) {
                negativeRevenue++;
                fixedIssues.push(`Fixed negative revenue (-$${Math.abs(row.revenue)})`);
            }
            if (row.revenue === 0)
                zeroRevenue++;
        }
        if (validDates < data.length * 0.9)
            warnings.push(`${data.length - validDates} rows have invalid dates`);
        if (negativeRevenue > 0)
            warnings.push(`${negativeRevenue} rows had negative revenue (fixed)`);
        if (zeroRevenue > data.length * 0.3)
            warnings.push(`High zero-revenue ratio: ${((zeroRevenue / data.length) * 100).toFixed(0)}%`);
        const channels = new Set(data.map(r => r.channel));
        for (const ch of channels) {
            if (!['Google Ads', 'Meta Ads', 'Microsoft Ads'].includes(ch))
                warnings.push(`Unknown channel: ${ch}`);
        }
        ...
        if (gaps > 5)
            warnings.push(`${gaps} gaps in date sequence`);
        return {
    ...
    generateQualityReport(validation) {
        ...
        if (validation.fixedIssues.length)
            lines.push('### Fixed:', ...validation.fixedIssues.map(f => `- ${f}`), '');
        lines.push('### Scores:', `- Completeness: ${(validation.dataQuality.completeness * 100).toFixed(0)}%`, `- Consistency: ${(validation.dataQuality.consistency * 100).toFixed(0)}%`, `- Accuracy: ${(validation.dataQuality.accuracy * 100).toFixed(0)}%`);
        return lines.join('\n');
    }
}
